...
export async function getNumberOfPairsContainsOther(inputFile: string) {
...
function zoneFullyOverlaps(overlappedZone: string[], zone: string[]) {
    ...
    if(startOverlappedZone >= startZone && startOverlappedZone <= endZone){
        return true
    }

    if(endOverlappedZone >= startZone && endOverlappedZone <= endZone){
        return true
    }
